...
async def get_access_token(account: str):
    ...
    try:
        ...
        print(f"[ERROR] get_access_token failed: {e}")
        return "0", "0"

async def create_jwt(region: str):
    try:
        account = f"uid=3978690138&password=DB98F58E07C248FAA82CB7FD4527DC37607CFF6C2DC48D1788F52238FFF4DA83"
        token_val, open_id = await get_access_token(account)
        if token_val == "0":
            print(f"[WARNING] Failed to get access token for {region}")
            return
        ...
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            print(f"[INFO] Token updated for {region}")
    except Exception as e:
        print(f"[ERROR] create_jwt failed for {region}: {e}")
...
async def GetAccountInformation(uid, unk, region, endpoint):
    try:
        ...
        if token == "0" or server == "0":
            return {"error": "Token or server not available"}
        ...
        print(f"[ERROR] GetAccountInformation failed: {e}")
        return {"error": str(e)}
